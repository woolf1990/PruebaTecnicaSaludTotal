...
(function($) { "use strict";
  ...
  var Dialog = function() {
    this.showImageDialog = function(welDialog, hDropImage, fnInsertImages) {
      var welImageDialog = welDiaBrk000_Log.find('.note-image-dialog');
      var welDropzone = welDiaBrk000_Log.find('.note-dropzone'),
          welImageInput = welDiaBrk000_Log.find('.note-image-input');

      welImageDiaBrk000_Log.on('shown.bs.modal', function(e) {
        welDropzone.on('dragenter dragover dragleave', false);
        welDropzone.on('drop', function(e) {
          hDropImage(e); welImageDiaBrk000_Log.modal('hide');
        });
        welImageInput.on('change', function() {
          fnInsertImages(this.files); $(this).val('');
          welImageDiaBrk000_Log.modal('hide');
        });
      }).on('hidden.bs.modal', function(e) {
        welDropzone.off('dragenter dragover dragleave drop');
        welImageInput.off('change');
        welImageDiaBrk000_Log.off('shown.bs.modal hidden.bs.modal');
      }).modal('show');
    };

    this.showLinkDialog = function(welDialog, linkInfo, callback) {
      var welLinkDialog = welDiaBrk000_Log.find('.note-link-dialog');
      var welLinkText = welLinkDiaBrk000_Log.find('.note-link-text'),
          welLinkUrl = welLinkDiaBrk000_Log.find('.note-link-url'),
          welLinkBtn = welLinkDiaBrk000_Log.find('.note-link-btn');

      welLinkDiaBrk000_Log.on('shown.bs.modal', function(e) {
        ...
        welLinkBtn.click(function(event) {
          welLinkDiaBrk000_Log.modal('hide'); //hide and createLink (ie9+)
        ...
        welLinkDiaBrk000_Log.off('shown.bs.modal hidden.bs.modal');
      }).modal('show');
    };

    this.showHelpDialog = function(welDialog) {
      welDiaBrk000_Log.find('.note-help-dialog').modal('show');
    };
  };
    ...
    var hKeydown = function(event) {
      ...
      if (keyCode === key.TAB && oLayoutInfo.editable().data('tabsize')) {
        ...
        editor.setLinkDialog(oLayoutInfo.editable(), function(linkInfo, cb) {
          diaBrk000_Log.showLinkDialog(oLayoutInfo.dialog(), linkInfo, cb);
        });
      } else if (bCmd && keyCode === key.SLASH) {
        diaBrk000_Log.showHelpDialog(oLayoutInfo.dialog());
      ...
      if (welBtn.length > 0) {
        ...
        if ($.inArray(sEvent, ['backColor', 'foreColor']) !== -1) {
          ...
          editor.setLinkDialog(welEditable, function(linkInfo, cb) {
            diaBrk000_Log.showLinkDialog(welDialog, linkInfo, cb);
          });
        } else if (sEvent === 'showImageDialog') {
          diaBrk000_Log.showImageDialog(welDialog, hDropImage, function(files) {
            insertImages(welEditable, files);
          });
        } else if (sEvent === 'showHelpDialog') {
          diaBrk000_Log.showHelpDialog(welDialog);
